...
class CNNLSTM(nn.Module):
    def __init__(self, input_dim, hidden_dim1, hidden_dim2, seq_len, output_dim):
        ...
        self.conv1 = nn.Conv1d(in_channels=input_dim, out_channels=64, kernel_size=3, stride=1)
        self.conv1.weight.data = self.conv1.weight.data.double()
        self.conv1.bias.data = self.conv1.bias.data.double()
...
class CNNGRU(nn.Module):
    def __init__(self, input_dim, hidden_dim1, hidden_dim2, seq_len, output_dim):
        ...
        self.gru1 = nn.GRU(input_size=pool2_output_size, hidden_size=hidden_dim1, batch_first=True, dtype=torch.double)
        self.gru2 = nn.GRU(input_size=hidden_dim1, hidden_size=hidden_dim2, batch_first=True, dtype=torch.double)
    ...
    def state_dict(self, destination=None, prefix="", keep_vars=False):
        return {
            "gru1": self.gru1.state_dict(destination=destination, prefix=prefix, keep_vars=keep_vars),
    ...
    def load_parameters(self, filename, save_flag=False):
        parameters = {}
        parameters.update({"gru1": self.gru1.state_dict()})
        parameters.update({"gru2": self.gru2.state_dict()})
        parameters.update({"conv1": self.conv1.state_dict()})
        parameters.update({"conv2": self.conv2.state_dict()})
        parameters.update({"fc1": self.fc1.state_dict()})
        parameters.update({"fc2": self.fc2.state_dict()})
